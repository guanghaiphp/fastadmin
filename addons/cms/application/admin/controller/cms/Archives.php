...
//            return json($result);
    ...
    {
        //这里无法监听到Archives的after_update事件
        \think\db\Query::event('after_update', function($options, $query) {
            if (isset($options['soft_delete']))
            {
                $channelIds = $query->column('channel_id');
                $channelNums = array_count_values($channelIds);
    ...
    public function move($ids = "")
    {
        if ($ids)
        {
            ...
            {
                $arc = $this->model->where('channel_id', 'in', function($query) use($channel) {
                            $query->name('channel')->where('model_id', $channel['model_id'])->field('id');
                        })->column('id,channel_id');
                $count = $this->model
                        ->where('id', 'in', array_keys($arc))
                        ->update(['channel_id' => $channel_id]);
                if ($count)
                {
                    $channelNums = array_count_values(array_values($arc));
                    foreach ($channelNums as $k => $v)
                    {
                        Channel::where('id', $k)->where('items', '>', 0)->setDec('items', $v);
                    }
                    Channel::where('id', $channel_id)->setInc('items', count($arc));
                    $this->success();
                }
                else
                {
