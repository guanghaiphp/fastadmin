...
class Channel Extends Model
{
    ...
    {
        return $value ? $value : self::$config['default_channel_img'];
    }
    ...
    public static function getChannelList($tag)
    {
        ...
        else if ($type === 'brother')
        {
            ...
            //同级
            $where['parent_id'] = ['exp', ' IN ' . $subQuery];
        }
        else if ($type === 'son')
        {
     ...
     * @return array
     */
    public static function getBreadcrumb($channel, $archives = [], $tags = [])
    {
        $list = [];
        $list[] = ['name' => __('Home'), 'url' => addon_url('cms/index/index')];
        if ($channel)
        {
    ...
    public static function getNav($channel, $tag = [])
    {
        ...
        return self::getTreeUl($tree, 0, $channel ? $channel['id'] : '', '', 1, $maxLevel);
    }
    ...
    public static function getTreeUl($tree, $myid, $selectedids = '', $disabledids = '', $level = 1, $maxlevel = 0)
    {
        ...
        if ($childs)
        {
            foreach ($childs as $value)
            {
                ...
                $itemtpl = '<li class="@dropdown" value=@id @selected @disabled><a data-toggle="dropdown" data-target="#" href="@url">@name @caret</a> @childlist</li>';
